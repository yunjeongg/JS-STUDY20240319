...
var tvxq = ['유노윤호', '최강창민', '영웅재중', '믹키유천', '시아준수'];
while (true) {
  /*배열안에 5명만 있지만 UserName을 잘못입력할 경우 몇번인지 정확히 모르기 때문에 
    while을 사용*/
  if (tvxq.length === 0) {
    alert ('모든 멤버가 삭제되었습니다');
    break;
  } else {
    var UserName = prompt (`현재멤버 : [${tvxq}]\n삭제할 멤버를 입력해주세요`);
    if (tvxq.includes(`${UserName}`)) {
      //삭제할 UserName이 있는 경우
      tvxq.splice(tvxq.indexOf(UserName), 1);
      alert (`삭제 완료!\n남은멤버:[${tvxq}]`);
    } else {
      //삭제할 UserName이 없는 경우
      alert (`${UserName}은(는) 잘못된 이름입니다. \n다시입력해주세요.`);
    }
  }
}
